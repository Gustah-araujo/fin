...
| --- | --------------------------------------------------------- | ---------- | ------------------------ |
| D-62 | Recurrence `type` immutable after creation | 2026-09-03 | REEX-01 spec; avoids cascading type changes; delete + recreate if type change needed |
...
| D-60 | Saldo projetado = Renda - Gasto (não considera saldo atual das contas) | 2026-09-02 | PLAN-01 design; foco em fluxo, não em saldo acumulado |
| D-61 | Evitar SQL raw; usar Eloquent + Collection para portabilidade entre databases | 2026-09-02 | PLAN-01 design; permite migração MariaDB → PostgreSQL sem reescrever queries |
...
| D-11 | ~~No linting tools~~ (superseded by D-51)                | 2026-07-13 | Simplicity preference    |
...
| D-51 | Quality gates: `composer quality` (Pint `--test` + PHPMD CyclomaticComplexity/NPathComplexity on `app/`) and `npm run quality` (ESLint `complexity` 20 + Prettier check). Auto-fix: `composer format`, `npm run format`, `npm run lint:fix` | 2026-08-19 | Supersedes D-11 |
| D-52 | Reusable `DataTable<T>` (React + shadcn `Table`) for tabular lists; lazy JSON fetch via axios; card grids (accounts/cards/categories/tags) unchanged | 2026-08-20 | DTBL-01 spec; list refactor |
| D-53 | `DatatableService` centralizes search/filter/sort/pagination + standardized JSON contract `{data, meta}` | 2026-08-20 | DTBL-01 spec; replaces per-controller filtering |
...
| 2026-08-19 | INCM-01 | Execute T10-T21 | FormRequests (Store/Update Income, Update Recurrence), IncomeController, RecurrenceController, routes, 5 Inertia pages (Incomes/Recurrences) + sidebar, 39 PHPUnit feature tests, 9 Cypress E2E tests. Scope edit/delete via ApplyRecurrenceScopeChangeJob (QUEUE_CONNECTION=sync in dev). Gates green: composer quality, npm run quality, 353 PHPUnit, npm run build, 46 Cypress |
| 2026-08-20 | DTBL-01 | Specify+Design | Reusable DataTable planned: lazy JSON via axios, `DatatableService` centralizing query+format, typed per-column filters (text/number/date/select) rendered in 2nd thead row, FK columns always select (uuid hidden). Decisions D-52–D-56. Migrate tabular lists (Transactions/Incomes/Recurrences/Members); card grids unchanged. Found pre-existing bug: category/account filter uuid-vs-int mismatch. |
| 2026-08-20 | DTBL-01 | Execute | DataTable infra built (shadcn table + axios, types/api/hook + 5 components), `DatatableService`+`DatatableConfig`+`Filter` (14 tests), migrated Transactions + Incomes + Recurrences (controller `datatable()` + JSON endpoints + page rewrites), RecurrenceResource `id`→`uuid`. Fixed uuid-vs-int FK filter bug. 383 PHPUnit (was 353), composer quality + npm run quality + build green. Members DEFERRED (role lives in `workspace_user` pivot; DatatableService is Builder-based — needs follow-up). Cypress E2E selectors pending update (row structure cards→table). |
| 2026-09-02 | PLAN-01 | Specify | Spec written: 5 user stories (PLAN-01 to PLAN-05), P3 deferred (orçamento por categoria). Key decisions: projeção 100% derivada de transações existentes (avulsas + parcelas + recorrências), sem entidade "despesa planejada", tela única com cards agregados + tabela mês-a-mês + modal de detalhamento, seletor de período com presets + custom date range, saldo projetado = renda - gasto (não considera saldo atual das contas). Milestone order swapped (D-57): Planejamento (P2) before Inteligência (P3). |
| 2026-09-02 | PLAN-01 | Design | Design written: PlanningService (getProjection + getMonthDetail), PlanningController (index + monthDetail), PlanningResource, Planning/Index.tsx (cards + tabela), MonthDetailModal.tsx. Arquitetura: projeção derivada de transações + recorrências projetadas em memória. Endpoint detalhamento via AJAX. 2 feature tests (PlanningServiceTest + PlanningControllerTest), 1 Cypress E2E. 8 arquivos novos, 2 modificados (routes/web.php, AppSidebar.tsx). Decisões D-58 a D-61 (D-61: evitar SQL raw, usar Eloquent + Collection groupBy para portabilidade). |
...
| 2026-09-11 | DTBL-02 | Execute | Todas as 9 tasks implementadas. Backend: TableStateService (7 tests), PersistsTableState trait (6 tests), DatatableStateController (5 tests), integração em Transaction/Income/Recurrence controllers (18 tests). Frontend: useDataTable hook + ActiveFilters component + DataTable mods + 3 pages integradas. Cypress E2E (5 cenários). 520 PHPUnit (1673 assertions), composer quality + npm run build green. ROADMAP DTBL-02 → 🟢 |
